...
describe("Search", () => {
  ...
  test.each([
    ...
    const button = wrapper.find(".mdi-magnify");
  ...
  test.each([
    ...
    const button = wrapper.find(".mdi-magnify");
    await button.trigger("click");
    const errorMessage = wrapper.find(".v-input--error .v-messages__message");
  ...
  test.each([
    ...
    async (validFilters, expected) => {
      ...
      wrapper.vm.setFilter(validFilters);
  ...
  test("Adds spaces when multiple filters are set", async () => {
    ...
    wrapper.vm.setFilter({ filter: "booleanFilter", type: "boolean" });
    wrapper.vm.setFilter({ filter: "stringFilter", type: "string" });
