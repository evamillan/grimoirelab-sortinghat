<template>
  ...
  >
    <td
      class="v-data-table-column--no-padding v-data-table-column--align-start"
    >
      <v-checkbox-btn
      ...
      <v-list-item class="pl-0">
        ...
        <v-list-item-title class="font-weight-medium">
          ...
          <v-tooltip location="bottom">
            ...
            <span>{{ isBot ? "Bot" : "Mark as bot" }}</span>
          </v-tooltip>
          ...
          <v-tooltip location="bottom">
            <template v-slot:activator="{ props }">
              <v-hover v-slot:default="{ isHovering }">
                ...
                >
                  {{ isHovering && isLocked ? "mdi-lock-open" : "mdi-lock" }}
                </v-icon>
              </v-hover>
            </template>
    ...
    <td width="112">
      <v-btn variant="text" size="small" icon @click.stop="$emit('expand')">
        <v-icon>
      ...
      <!-- <v-tooltip location="bottom">
        <template v-slot:activator="{ props }">
          <v-icon size="small" :disabled="isLocked" v-bind="props" class="ml-2">
            mdi-drag-vertical
          </v-icon>
        </template>
