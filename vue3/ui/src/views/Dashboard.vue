<template>
  <v-main class="ma-md-8">
    <work-space
      ...
      @updateStore="workspaceStore.save($event)"
    />
    <v-row class="ma-md-0 mt-md-8">
      <individuals-table
        class="individuals"
...
export default {
  ...
  async mounted() {
    if (
      ...
      this.workspaceStore.workspace.length > 0
    ) {
      const response = await Promise.all(
        this.workspaceStore.workspace.map((uuid) =>
          getIndividualByUuid(this.$apollo, uuid)
        )
      );
